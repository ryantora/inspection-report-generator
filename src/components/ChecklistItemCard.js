...
//  */
// export default function ChecklistItemCard({ entry, optional, onCapture, onUpload, onMarkNA, onRetake, onPressImage }) {
 ...
 * entry: { item_id, item_title, image_uri, status } where status is
 * 'missing' | 'completed' | 'na'
 * displayNumber: the sequential number to print (e.g. "5.") — computed by
 * db.js:getCombinedOrderedEntries based on final position, since dynamic
 * camera sections mean entry.item_id is no longer necessarily the same as
 * the item's position in the list.
 * showAddCamera/onAddCamera: shown instead of Mark N/A on the LAST Dome
 * Camera section, letting the inspector add another camera (C2, C3, ...)
 * directly below rather than the old "mark unused cameras N/A" approach.
 */
export default function ChecklistItemCard({
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
      <View style={styles.headerRow}>
        <Text style={[styles.title, { color: theme.text }]} numberOfLines={3}>
          {displayNumber ?? entry.item_id}. {entry.item_title}
        </Text>
        <View style={[styles.statusPill, { backgroundColor: statusColor }]}>
          <Text style={styles.statusText}>{statusLabel}</Text>
        </View>
      ...
      <View style={styles.secondaryRow}>
        ...
        {showAddCamera && (
          <TouchableOpacity onPress={onAddCamera}>
            <Text style={[styles.secondaryLink, { color: theme.primary }]}>+ Add Camera</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
