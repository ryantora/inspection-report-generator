    ...
    <head>
      ...
      <style>
        ...
        .page {
          ...
          padding-top: 4mm;
          /* iOS/WebKit's print engine (used by expo-print on iOS) is more
             prone than Chromium (Android) to splitting a page's content
             across the page boundary mid-image unless BOTH the legacy and
             modern CSS fragmentation properties are set — page-break-inside
             alone isn't reliably honored on WebKit for tall content. */
        ...
        .item-image {
          ...
          width: 100%;
          height: auto;
          /* Hard cap so an image can never be taller than one page's
             available content height (A4 257mm minus header/title/padding
             above it, with a safety margin) — combined with the
             break-inside rules above, this is what stops iOS from
             splitting a photo across two pages, which doesn't happen on
             Android/Chromium. */
          max-height: 180mm;
          object-fit: contain;
          page-break-inside: avoid;
